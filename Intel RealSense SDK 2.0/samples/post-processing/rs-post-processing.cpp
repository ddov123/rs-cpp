...
int main(int argc, char* argv[]) try
{
    ...
    window app(1280, 720, "Hkit 3D Scan");
    ...
    while (app)
    {
        ...
        if (capture)
        {
            ...
            {
                colored_depth = color_map.process(f);                                // Colorize the depth frame with a color map
                filtered_pc.map_to(colored_depth);                                   // Map the colored depth to the point cloud
                filtered_points = filtered_pc.calculate(f);                          // Generate pointcloud from the depth data
